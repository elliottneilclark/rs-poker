    ...
    R: Rng + SeedableRng,
{
    ...
    /// iterations and actions using rayon. Sub-agents inherit the pool for
    /// nested parallelism. Rayon's work-stealing handles this without thread explosion.
    ...
    R: Rng + SeedableRng + Send + 'static,
{
    ...
    /// parallel contexts where `self` cannot be borrowed. All needed state is
    ...
    fn compute_reward(
        ...
        let (_before_node_idx, _before_child_idx, player_idx) = traversal_state.get_all();
        ...
        let forked_traversal_set = traversal_set.fork();

        let sub_depth = depth + 1;

        // Clone Arc configs (cheap atomic increment) instead of deep-cloning
        let iter_config = iter_gen_config.clone();
        let action_config = action_gen_config.clone();
        let cached_mapper_config = *action_index_mapper.config();
        ...
        for (i, cfr_state_i) in cfr_states.iter().cloned().enumerate() {
            ...
            let mut builder = CFRAgentBuilder::<T, I, R>::new()
                ...
                .cfr_states_arc(cfr_states.clone())
        ...
        {
            let (after_node_idx, after_child_idx) = traversal_state.get_position();
            assert_eq!(
    ...
    pub fn explore_all_actions(&mut self, game_state: &GameState) {
        ...
        if let Some(pool) = &self.thread_pool {
            ...
            // Destructure self fields for Sync closure capture.
            // CFRAgent is not Sync (StdRng is not Sync), but all these fields are.
            let traversal_set = &self.traversal_set;
            let traversal_state = &self.traversal_state;
            let cfr_states = &self.cfr_states;
            let iter_gen_config = &self.iter_gen_config;
            let action_gen_config = &self.action_gen_config;
            let action_index_mapper = &self.action_index_mapper;
            ...
            let results: Vec<(usize, usize, f32)> = pool.install(|| {
                task_rngs
                    .into_par_iter()
                    .enumerate()
                    .map(|(task_id, mut rng)| {
                        ...
                        let reward_idx = action_index_mapper.action_to_idx(action, game_state);
                        let reward = Self::compute_reward(
                            ...
                            action_index_mapper,
                            depth,
                            limited_exploration_depth,
                            allow_node_mutation,
                        );
                        (iter_idx, reward_idx, reward)
                    })
                    .collect()
            });
            ...
            for iter_idx in 0..num_iterations {
                ...
                for &(i, reward_idx, reward) in &results {
                    if i == iter_idx {
            ...
            // Sequential path (unchanged) — used when no thread pool is configured.
            ...
            for _ in 0..num_iterations {
                ...
                for action in &actions {
                    ...
                    rewards[reward_idx] = Self::compute_reward(
                        game_state,
                        action,
                        &mut self.rng,
                        &self.traversal_set,
                        ...
                        &self.cfr_states,
                        &self.iter_gen_config,
                        &self.action_gen_config,
                        &self.action_index_mapper,
                        self.depth,
                        self.limited_exploration_depth,
                        self.allow_node_mutation,
                    );
                }
