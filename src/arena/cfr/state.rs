...
impl CFRState {
    ...
    pub fn add(&self, parent_idx: usize, child_idx: usize, data: NodeData) -> usize {
        ...
        self.arena.get(parent_idx).set_child(child_idx, idx);
    ...
    /// 4. If CAS fails, another thread won — use their node (dead slot is fine)
    ///
    /// # Arguments
    ///
    /// * `parent_idx` - The index of the parent node
    ...
    pub fn ensure_child(
        ...
        if let Some(existing_idx) = self.arena.get(parent_idx).get_child(child_idx) {
            ...
            let node = Node::new(parent_idx, child_idx, expected_data);
            let idx = self.arena.push(node);

            // Use try_set_child (CAS) to handle concurrent creation.
            // If another thread beat us, use their node index instead.
